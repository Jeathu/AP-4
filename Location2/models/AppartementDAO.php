...
class AppartementDAO {
    ...
    public function createApartment(Appartement $appartement) {
        $stmt = $this->pdo->prepare("INSERT INTO appartement (nom_appart, adresse, code_postal, ville, etage, typappart, prix_loc, prix_charg, ascenseur, date_libre) 
                      VALUES (:nom_appart, :adresse, :code_postal, :ville, :etage, :typappart, :prix_loc, :prix_charg, :ascenseur, :date_libre)");
        try {
        $stmt->execute([
            ...
            'typappart' => $appartement->getTypappart(),
            'prix_loc' => $appartement->getPrix_loc(),
            'prix_charg' => $appartement->getPrix_charg(),
    ...
    public function getByOwner($ownerId) {
        try {
            $stmt = $this->pdo->prepare("SELECT * FROM appartement WHERE proprietaire_id = :ownerId");
